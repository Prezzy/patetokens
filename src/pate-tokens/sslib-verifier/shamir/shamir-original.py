...
def lagrange_interpolation(x, points, prime_mod):
    ...
    for i, (xi, yi) in enumerate(points):
        print("Points in the thing {}".format((xi,yi)))
        numerator = yi
        denominator = 1
        print("xi {}".format(xi))
        ...
        temp = (numerator*util.modular_inverse(denominator, prime_mod)) % prime_mod
        print("I believe this is the Lagrange Coefficient {}".format(temp))
...
def lagrange_coefficient(x, point, indexs, prime_mod):
    ...
    print("point in my thing {}".format((xi,yi)))
    numerator = yi
    denominator = 1
    print("my xi {}".format(xi))
    for j, (xj) in enumerate(indexs):
        if xj == xi:
            continue
        print("my xj is {}".format(xj))
        numerator *= (x - xj + prime_mod) % prime_mod
        numerator %= prime_mod
        denominator *= (xi - xj + prime_mod) % prime_mod
        denominator %= prime_mod
    y = (numerator*util.modular_inverse(denominator, prime_mod)) % prime_mod
    print("This is my Lagrange Coefficient {}".format(y))
    #y %= prime_mod
...
def split_secret(secret_bytes, required_shares, distributed_shares, **kwargs):
    ...
    largest_representable_secret = util.int_from_bytes(secret_bytes) #util.int_from_bytes(bytes([255]) * secret_length)
    ...
    prime_bytes = int.bit_length(prime_mod-1)//8
...
def recover_secret(data):
    ...
    if not isinstance(prime_mod, int):
        print(type(prime_mod))
        print("NEED INT PRIME")
